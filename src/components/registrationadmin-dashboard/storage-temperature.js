...
const StorageTemperatureArea = () => {
  ...
  if (id === null) {
    ...
    console.log("account_id on Storage temperature page is:", id);
  }
  ...
  const totalPages = Math.ceil(storagetemperaturename.length / itemsPerPage);
  // Api Path
  ...
  const fetchStoragetemperaturename = async () => {
    try {
      const response = await axios.get(
        `${url}/storagetemperature/get-storagetemperature`
      );
  ...
  const currentData = storagetemperaturename.slice(
  ...
  const handleFilterChange = (field, value) => {
    if (value === "") {
      fetchStoragetemperaturename(); // Reset to fetch original data
    } else {
      // Filter the sample array based on the field and value
      const filtered = storagetemperaturename.filter((storagetemperature) =>
        storagetemperature[field]
          ?.toString()
          .toLowerCase()
          .includes(value.toLowerCase())
      );
      setStoragetemperaturename(filtered); // Update the state with filtered results
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${url}/storagetemperature/post-storagetemperature`,
        formData
      );
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(
        `${url}/storagetemperature/delete-storagetemperature/${selectedstoragetemperaturenameId}`
      );
      console.log(
      ...
      console.error(
        `Error deleting Storage temperature with ID ${selectedstoragetemperaturenameId}:`,
        error
      );
    }
  };
  ...
  const handleEditClick = (storagetemperaturename) => {
    ...
    setFormData({
      storagetemperaturename: storagetemperaturename.name,
      added_by: id,
    });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${url}/storagetemperature/put-storagetemperature/${selectedstoragetemperaturenameId}`,
        formData
      );
      console.log(
      ...
      setSuccessMessage("Storage temperature updated successfully.");
      ...
      console.error(
        `Error updating Storage temperature name with ID ${selectedstoragetemperaturenameId}:`,
        error
      );
    }
  };
              ...
              <div className="d-flex flex-column w-100">
                ...
                <div className="d-flex justify-content-end align-items-center gap-2 w-100">
                  ...
                  <label className="btn btn-secondary mb-2">
                    Upload Storage temperature List
                    <input
                      type="file"
                      accept=".xlsx, .xls"
                      style={{ display: "none" }}
                          ...
                          <div className="modal-body">
                            ...
                            <div className="form-group">
                              ...
                              <input
                                ...
                                name="storagetemperaturename" // Fix here
                                value={formData.storagetemperaturename}
