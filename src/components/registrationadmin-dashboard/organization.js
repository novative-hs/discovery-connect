...
const OrganizationArea = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/organizations/post",
        formData
      );
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:5000/api/admin/organization/delete/${selectedOrganizationId}`
      );
      console.log(
        `Organization with ID ${selectedOrganizationId} deleted successfully.`
      );
