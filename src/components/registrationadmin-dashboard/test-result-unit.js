...
const TestResultUnitArea = () => {
 ...
 const [showAddModal, setShowAddModal] = useState(false);
  ...
  const currentData = filteredTestResultunitname.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error adding Test Result Unit", error);
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      console.error(`Error updating Test Result Unit: ${selectedTestResultUnitnameId}`, error);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      console.error(`Error deleting Test System: ${selectedTestResultUnitnameId}`, error);
    }
  };

  const handleEditClick = (item) => {
    setSelectedTestResultUnitnameId(item.id);
    setEditTestResultUnitname(item);
  ...
  const handleFileUpload = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        console.error("Error uploading test result unit", error);
      }
    };
