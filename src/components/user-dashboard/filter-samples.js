...
const FilterProductArea = ({ selectedProduct, closeModals }) => {
  ...
  if (isInCart(sample.id)) return; // prevent duplicate
              ...
              <table className="table table-bordered table-hover text-center align-middle w-auto border">
                <thead className="table-primary text-dark">
                  <tr>
                    ...
                    <th className="p-2 text-center" style={{ minWidth: "30px" }}>Action</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    currentData.map((sample) => (
                      <tr key={sample.id}>
                        {tableHeaders.map(({ key }, idx) => {
                          let content;
                          if (key === "price") {
                            content = sample.price ? `${sample.price} ${sample.SamplePriceCurrency || ""}` : "Request the Quote";
                          } else if (key === "Analyte") {
                            content = (
                              <span
                            ...
                            content = `${sample.quantity || 1} x ${sample.volume || "----"}${sample.VolumeUnit || ""}`;
                          } else if (key === "age") {
                          ...
                          return (
                            <td
                              key={idx}
                              className={key === "price" ? "text-end" : key === "Analyte" ? "" : "text-center text-truncate"}
