          ...
          <p className="mb-1">Not Paid</p> */}
              ...
              <tr className="total-row">
                ...
                <td className="text-end fw-bold" style={{ paddingRight: '16px' }}>
                  {(function () {
                    const total = cart_products.reduce((sum, item) => {
                      const price = Number(item.price) || 0;
                    ...
                    // get currency code from the product (fallback to PKR)
                    const currencyCode = cart_products[0]?.SamplePriceCurrency || "PKR";

                    // get correct symbol (fallback to the code itself if not in the map)
