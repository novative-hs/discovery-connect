...
const fetchSamples = async ({ queryKey: [, id, page, pageSize, field, value] }) => {
  ...
  const orders = (ordersRes.data.results || []).filter(
    (o) => (o.committee_status || "").toLowerCase() === "pending"
  );
...
const SampleArea = () => {
  ...
  const handleViewDocument = useCallback((buf, name) => {
    ...
    const url = URL.createObjectURL(
      new Blob([new Uint8Array(buf.data)], { type: "application/pdf" })
    );
    window.open(url, "_blank");
    setViewedDocuments((v) => ({ ...v, [name]: true }));
  }, []);

  // ✅ Check current group's required docs rather than previous state
  const allDocsViewed = (group) => {
    const first = group?.[0];
    if (!first) return false;
    const reqOK = viewedDocuments.study_copy; // required
    const optOK = !first?.nbc_file || viewedDocuments.nbc_file; // optional: only if exists
    return reqOK && optOK;
  };

  const handleOpenModal = (type, group) => {
    if (!group?.length) {
      alert("Sample data missing.");
      return;
    }
    if (!allDocsViewed(group)) {
      alert("Please view all required documents first.");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!id || !selectedResearcherSamples?.length || !text) {
  ...
  const renderCell = (k, s) => {
    if (k === "Analyte")
      return <span style={{ textDecoration: "underline" }}>{s.Analyte || "N/A"}</span>;
    if (k === "quantity")
      return `${s.quantity || 0} × ${s.volume || 0}${s.VolumeUnit || ""}`;
    if (k === "TestResult")
      return s.TestResult || s.TestResultUnit
        ? `${s.TestResult ?? ""} ${s.TestResultUnit ?? ""}`.trim()
        : "----";
    if (k === "age") return s.age != null ? `${s.age}` : "---";
    if (k === "gender") return s.gender || "---";
    if (k === "reporting_mechanism" || k === "comments")
      return s[k] ? renderLongText(s[k]) : "----";
    if (k === "locationids")
      return (
        <span title="Location ID's = Room Number, Freezer ID and Box ID">
          {s[k] || "----"}
        </span>
      );
  ...
  return (
    <div className="container py-3">
      ...
      <div className="table-responsive">
        <table className="table table-bordered table-hover text-center align-middle">
          <thead className="table-primary text-dark">
            <tr>
              {["Order ID", "Order Date", "Researcher Name", "Organization Name", "Review Status"].map(
                (label, i) => (
                  <th key={i}>
                    <div className="d-flex flex-column align-items-center">
                      <input
                        ...
                        onChange={(e) => {
                          setSearchField(label.toLowerCase().replace(" ", "_"));
        ...
        >
          ...
          <Modal.Body>
            ...
            <div className="mb-3 d-flex flex-wrap gap-2">
              {docKeys.map((k) => (
                <button
                  ...
                  onClick={() =>
                    handleViewDocument(selectedResearcherSamples[0][k], k)
                  }
                >
                  Download {k.replace("_", " ").toUpperCase()}{" "}
            ...
            {selectedResearcherSamples.some(
              (s) => s.committee_status !== "Approved" && s.committee_status !== "Refused"
            ) && (
                          ...
                          <td
                            ...
                            onClick={(e) => {
                              if (key === "Analyte") {
                                ...
                                setShowSampleModal(true); // ✅ correct setter
                              }
                            }}
                          >
                            {renderCell(key, sample)}
                          </td>
                        ))}
                      </tr>
                    ))
      ...
      <Modal
        show={showSampleModal && !!selectedSample}
        onHide={() => setShowSampleModal(false)}
        size="lg"
        centered
      >
        <Modal.Header closeButton>
          <Modal.Title className="fw-bold">{selectedSample?.Analyte}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="row">
